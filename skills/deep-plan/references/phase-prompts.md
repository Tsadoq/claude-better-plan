...
4. Synthesis: merge perspectives into a single plan body using
   ...
   **Tests (TDD)** subsection only for tasks that produce or modify code, carrying
   ...
   authoring rules of references/test-principles.md; omit it for markdown, docs, or
   config tasks. Append the Phase 3 dossiers verbatim under a ## Research dossiers
   appendix, opening it with the template's ### Coverage preamble table (one row per
   ## Decisions made row, naming the dossier or why the decision was not researched),
   so they survive into the archived folder members.
...
**Tests (TDD)** block. And run it once more with agentType
deep-plan:dp-readability-critic: one finder per H3 cluster of ## Review-time red
flags in readability-principles.md, reviewing plan.md, design.md, and
architecture.md when present as documents. Design, test, and readability findings
carry the same material|minor tags, merge into the handling below, and share the
depth loop bounds; no separate knobs.
