...
# --- the always-resident descriptions --------------------------------------
#
# A skill's or agent's frontmatter `description` is in context from session
# start whether or not the thing is ever used, so these strings are the only
# part of this plugin every session pays for. Two limits apply and they fail
# for different reasons, which is why each has its own assertion below: the
# character cap is the harness's, and crossing it truncates the tail silently;
# the word budget is this project's own routing target, which nothing enforces
# at runtime.
#
# Globbed rather than listed, so a new skill or agent is inside the budget the
# day it is added. This is a size check and not a frontmatter-schema one -- which
# fields the implementer agent must declare is `test_agents_contract.py`'s -- but
# an unreadable or empty description is still reported, because a measurement
# taken on nothing is under every budget and would pass for the wrong reason.
ALWAYS_RESIDENT = tuple(
    str(path.relative_to(REPO))
    for path in sorted((REPO / "skills").glob("*/SKILL.md")) + sorted((REPO / "agents").glob("*.md"))
)

# Same reason as CONTRACT_TESTS below: an empty parametrisation reports as "no
# tests ran" rather than as a failure.
assert ALWAYS_RESIDENT, f"no skills or agents found under {REPO}"

# `description: |` leaves the block-scalar indicator on the key's own line, and
# `frontmatter_value` returns the raw value including it. It is YAML syntax, not
# prose, so it comes off before measuring; counted, it would report every folded
# description as one word and one character longer than it is. The indicator is
# matched at end-of-string as well as before whitespace, so an *empty* block
# scalar reduces to nothing and trips the emptiness check below rather than
# measuring as a one-word description.
_BLOCK_SCALAR = re.compile(r"^[|>][+-]?\d*(?:\s|$)")


def _description(path: Path) -> str:
    """The file's `description` as the harness sees it: one normalised line.

    Whitespace is collapsed because both YAML block styles turn the folded
    source lines into single separators, so the measured length matches the
    value the field cap applies to rather than the source's indentation.
    """
    raw = guarantees.frontmatter_value(path.read_text(encoding="utf-8"), "description") or ""
    description = " ".join(_BLOCK_SCALAR.sub("", raw.strip(), count=1).split())
    assert description, (
        f"{path.relative_to(REPO)} has no frontmatter `description` with any text in "
        f"it, so there is nothing to measure and every budget below would pass "
        f"vacuously. A skill or agent without one is also unroutable"
    )
    return description


@pytest.mark.parametrize("relative_path", ALWAYS_RESIDENT, ids=lambda p: p)
def test_descriptions_fit_their_budget(relative_path: str) -> None:
    description = _description(REPO / relative_path)

    cap = guarantees.BUDGETS["description_chars"]
    assert len(description) <= cap, (
        f"{relative_path}: `description` is {len(description)} characters, over the "
        f"{cap}-character field cap. Past the cap the harness truncates, so whatever "
        f"routing keywords sit at the tail stop reaching the router: shorten the "
        f"description. Raising BUDGETS['description_chars'] is not an option -- it is "
        f"the harness's number, not ours"
    )
